...
import {
  ...
  BookOpen, FileQuestion, TrendingUp, Lightbulb, ChevronDown, Loader2
} from 'lucide-react';
      ...
      {lines.map((line, i) => {
        ...
        const rendered = parts.map((p, j) =>
          p.startsWith('**') && p.endsWith('**')
            ? <strong key={j} className="font-semibold" style={{ color: 'var(--color-text)' }}>{p.slice(2, -2)}</strong>
            : p
        );
        // Bullet points
        ...
        // Headings ###
        if (line.startsWith('### ')) {
          return <p key={i} className="font-bold text-base mt-2" style={{ color: 'var(--color-text)' }}>{line.slice(4)}</p>;
        }
        if (line.startsWith('## ')) {
          return <p key={i} className="font-bold text-lg mt-3" style={{ color: 'var(--color-text)' }}>{line.slice(3)}</p>;
        }
...
export default function ChatbotPage() {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const res = await chatbotService.sendMessage(msg, getHistory(), selectedModel);
      ...
      const errorMsg =
        ...
        'Connection failed. Make sure OPENROUTER_API_KEY is set in backend .env';
      setMessages(prev => [...prev, {
        id:      Date.now() + 1,
        role:    'assistant',
        text:    `⚠️ ${errorMsg}`,
          ...
          <div className="relative">
            ...
            <AnimatePresence>
              {showModelPicker && (
                ...
                >
                  <div className="p-2">
                    <p className="text-xs font-semibold px-2 py-1.5 mb-1"
          ...
          {messages.map((msg) => (
            ...
            >
              ...
              <div className={`flex flex-col gap-1 max-w-[80%]
                ...
                <div className={`flex items-center gap-2 px-1 opacity-0 group-hover:opacity-100
